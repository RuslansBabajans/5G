...
SNR=200; % AWGN signal-to-noise ratio in dB
...
txImpPhs=0*pi/180; % Tx IQ-imbalance phase in radians -pi/12...pi/12
...
window=1000;
compAngle=0;
rxImpPhsComp=zeros(1,length(usrRxImp)-window);
angle_step=5e-3;     %5e-4;
usrRxImp2=zeros(1,length(usrRxImp)-window);
corrCoef=zeros(1,length(usrRxImp)-window);


for k=1:1:length(usrRxImp)-window
    
    % Current compensation angle
    rxImpPhsComp(k)=compAngle*pi/180;
    
    % Apply compensation
    usrRxImp2(k:k+window)=cos(rxImpPhsComp(k))*real(usrRxImp(k:k+window))-sin(rxImpPhsComp(k))*imag(usrRxImp(k:k+window))+1j*(-sin(rxImpPhsComp(k))*real(usrRxImp(k:k+window))+cos(rxImpPhsComp(k))*imag(usrRxImp(k:k+window)));
   
    % Calculate correlation between I and Q
    corrCoef(k)=corr2(real(usrRxImp2(k:k+window)),imag(usrRxImp2(k:k+window)));
    
    % Determine how to modify compensation angle based on correlation
    if corrCoef(k)<0
       compAngle=compAngle-angle_step*abs(corrCoef(k));
    else
        compAngle=compAngle+angle_step*abs(corrCoef(k));
    end
end

% Create vector of the original distortion angle
rxImpPhsDist=zeros(1,length(usrRxImp)-window)+rxImpPhs*180/pi;

% Calculate difference between original distortion and compensation angles
delta=abs(rxImpPhsDist)-abs(rxImpPhsComp*180/pi);

% Output compensated signal
usrRxImp =  usrRxImp2;
...
figure()
hold on
plot(rxImpPhsDist,'.-','color','#0072BD')
plot(rxImpPhsComp*180/pi,'.-','color','#D95319')
plot(delta,'.-','color','#77AC30')
plot(-rxImpPhsDist,'k--')
plot(zeros(size(rxImpPhsDist)),'k--')
grid on, grid minor
ylabel("Phase, deg")
xlabel("Samples")
legend("Phase impairment angle","Compensation angle","Angle compensation \Delta")
ylim([-12 12])
