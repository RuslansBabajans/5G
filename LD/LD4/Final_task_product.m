...
rxImpPhs=15*pi/180; % Rx IQ-imbalance phase in radians -pi/12...pi/12
...
window=1000;
compAngle=0;
rxImpPhsComp=zeros(1,length(usrRxImp));
angle_step=1e-3;
usrRxImp2=zeros(1,length(usrRxImp));
product=zeros(1,length(usrRxImp));

for k=1:1:length(usrRxImp)
    
    % Current compensation angle
    rxImpPhsComp(k)=compAngle*pi/180;
    
     % Apply compensation
    usrRxImp2(k)=cos(rxImpPhsComp(k))*real(usrRxImp(k))-sin(rxImpPhsComp(k))*imag(usrRxImp(k))+1j*(-sin(rxImpPhsComp(k))*real(usrRxImp(k))+cos(rxImpPhsComp(k))*imag(usrRxImp(k)));
    
    % Calculate product=I*Q
    product(k)=real(usrRxImp2(k)).*imag(usrRxImp2(k));
    
    % Determine how to modify compensation angle based on product
    if product(k)<0
       compAngle=compAngle-angle_step*abs(product(k));
    else
        compAngle=compAngle+angle_step*abs(product(k));
    end    
end
...
rxImpPhsDist=zeros(size(usrRxImp))+rxImpPhs*180/pi;
