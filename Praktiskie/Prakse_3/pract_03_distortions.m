...
BETA=0; % AM/AM nonlinearity parameter 0...1
GAMMA=0; % AM/PM nonlinearity parameter -2...2

rxImpPhs=0*pi/180; % Rx IQ-imbalance phase in radians -pi/12…pi/12
...
usrTxImp=cos(txImpPhs)*real(usrTxImp)+sin(txImpPhs)*imag(usrTxImp)+1j*(sin(txImpPhs)*real(usrTxImp)+cos(txImpPhs)*imag(usrTxImp)); % expressions from lecture, wierd IQ
...
figure(1); axis square; hold on; grid on
plot(usrDeRot(3:4:end),'b.')
plot(ALPH,'ro')
xlim([-4 4]); ylim([-4 4])
return
